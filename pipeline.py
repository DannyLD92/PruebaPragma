...
url= '.\\' #downloads\\Copia de dataPruebaDataEngineer (2) (3)\\dataPruebaDataEngineer\\' 
# cambiar URL de archivo
...
conn = oc.connect('Driver={SQL Server};'
                 'Server=localhost\SQLEXPRESS;' #servidor
                 'Database=Prueba;' #DB
...
for archivo in contenido :
    if os.path.isfile(os.path.join(url, archivo)) and archivo.endswith('.csv') and archivo.startswith('2012'): #define que archivos tomar
        ...
        with open(url + "" + archivo_csv, 'r') as csvfile: #apertura de archivo
            ...
            archivo= ("INSERT INTO Validacion(archivo) values('"+ archivo_csv+"')") #Insercion de validacion en la bd
            cursor.execute(archivo)
            conn.commit()
            fecha = ''
            id = ''
            valor = ''
            ...
            for row in  spamreader  :      #bucle para leer el archivo de inicio a fin
                ...
                actuaArc= ("update Validacion set conteo = " + str(contador) +" , maximo = " + str(maximo) +" , minimo = " + str(minimo) +
                           " , promedio = " + str(promedio) + " , medio= "+ str(medio) + " , bandera = 'Exito' where archivo = '"+ archivo_csv +"' "   )
                cursor.execute(actuaArc)
                conn.commit()
